...
namespace CapaDatos.ContabilidadAPI.Models
{
    public class ComprobanteExtractorResult
    {
        ...
        /// <summary>Suma de TaxableAmount (base imponible) a tasa reducida, para uso en CreateAsync y frontend (igual semántica que MontosGravados)</summary>
        public List<string> MontosIgvEspecial { get; set; } = new();
        /// <summary>Suma de TaxAmount (impuesto calculado por SUNAT) a tasa reducida — usado por el background service para guardar el IGV exacto</summary>
            ...
            try
            {
                ...
                foreach (var taxTotal in taxTotalsList)
                {
                    ...
                    foreach (var taxSubtotal in taxSubtotalsList)
                    {
                        ...
                        var taxAmount = taxSubtotal.Element(cbc + "TaxAmount")?.Value;
                            ...
                            switch (taxExemptionReasonCode)
                            {
                                case "10": // Gravado - VALIDAR PORCENTAJE
                                            ...
                                            System.Globalization.CultureInfo.InvariantCulture, out decimal percent))
                                    {
                                        if (percent < 18m)
                                        {
                                            Console.WriteLine($"[XML] IGV Especial ({percent}%) detectado: {taxableAmount}");
                                    ...
                                    {
                                        // Sin porcentaje o no parseable → asumir gravado normal (18%)
                                        Console.WriteLine($"[XML] Gravado (sin % o no parseable, asumiendo 18%) detectado: {taxableAmount}");
                                        result.MontosGravados.Add(taxableAmount);
                                    }
                                    if (!string.IsNullOrEmpty(taxAmount))
                                        result.MontoIgvDocumento = taxAmount;
                                ...
                                switch (lineTaxExemptionReasonCode)
                                {
                                    case "10": // Gravado - VALIDAR PORCENTAJE
                                                ...
                                                System.Globalization.CultureInfo.InvariantCulture, out decimal linePercent))
                                        {
                                            if (linePercent < 18m)
                                            {
                                                // MontosIgvEspecial = base imponible (TaxableAmount), igual semántica que MontosGravados
                                                // MontosBaseIgvEspecial = TaxAmount real de SUNAT, usado por el background service
                                                Console.WriteLine($"[XML-LINE] IGV Especial ({linePercent}%) - Base={lineTaxableAmount}, TaxAmount={lineTaxAmount}");
                                                result.MontosIgvEspecial.Add(lineTaxableAmount);
                                                if (!string.IsNullOrEmpty(lineTaxAmount))
                                                    result.MontosBaseIgvEspecial.Add(lineTaxAmount);
                                                afectacionDetectada = true;
                                            }
